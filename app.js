...
class Dictionary {
    ...
    async handleSearch(options = {}) {
        ...
        try {
            ...
            const localResults = searchDictionary(query);
            const enhancedLocalResults = localResults.map(result => ({
                ...result,
                source: window.offlineDictionary ? 'offline' : 'local',
                sourceLabel: window.offlineDictionary ? 'Offline Dictionary' : 'Local Data',
                isOfflineFallback: true
            }));
            this.currentSearchResults = enhancedLocalResults;
            this.displayResults(enhancedLocalResults, query, { showOfflineMessage: true });
    ...
    async fetchArabicTranslation(text) {
        try {
            ...
            const translationMethods = [
                () => this.translateViaGoogleAPI(text, 'en', 'ar'),
                () => this.translateViaLibreTranslate(text, 'en', 'ar'),
                () => this.translateViaMyMemory(text, 'en', 'ar')
            ];
            ...
            for (const method of translationMethods) {
                try {
                    ...
                    if (result) {
                        console.log('Arabic translation success:', result);
                        return result;
                    }
                } catch (error) {
            ...
            console.error('All translation methods failed for Arabic translation');
            return null;
        } catch (error) {
    ...
    async fetchEnglishTranslation(text) {
        try {
            ...
            // Try multiple translation APIs for better reliability
            const translationMethods = [
                () => this.translateViaGoogleAPI(text, 'ar', 'en'),
                () => this.translateViaMyMemory(text, 'ar', 'en'),
                () => this.translateViaLibreTranslate(text, 'ar', 'en')
            ];
            ...
            for (const method of translationMethods) {
                try {
                    const result = await method();
                    if (result) {
                        console.log('English translation success:', result);
                        return result;
                    }
                } catch (error) {
                    console.warn('Translation method failed:', error);
            ...
            console.error('All translation methods failed for English translation');
            return null;
        } catch (error) {
            console.error('Translation error:', error);
            return null;
    ...
    // Google Translate API (unofficial but working)
    async translateViaGoogleAPI(text, sourceLang, targetLang) {
        try {
    ...
    // MyMemory Translation API (free without API key)
    async translateViaMyMemory(text, sourceLang, targetLang) {
        try {
            const encodedText = encodeURIComponent(text.trim());
    ...
    // LibreTranslate API (backup - requires API key for public instance)
    async translateViaLibreTranslate(text, sourceLang, targetLang) {
        try {
    ...
    combineResults(localResults, apiResults) {
        ...
        // Add local results first with clear source indication
        ...
        apiResults.forEach(result => {
            ...
            if (!seen.has(key)) {
                ...
                combined.push({
                    ...result,
                    source: 'api',
                    sourceLabel: result.source || 'Online API'
                });
            }
        });
    ...
    displayResults(results, query, options = {}) {
        ...
        if (results.length === 0) {
            const isOffline = !navigator.onLine || options.showOfflineMessage;
            this.resultsContainer.innerHTML = `
                <div class="no-results">
                    <div class="no-results-icon">🔍</div>
                    <h3>No results found</h3>
                    <p>Try searching for a different word or check your spelling</p>
                    ${isOffline ? '<div class="offline-notice" style="margin-top: 15px; padding: 10px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; color: #856404;"><strong>📚 Offline Mode:</strong> Limited to offline dictionary entries. Connect to internet for comprehensive results.</div>' : ''}
                </div>
            `;
            return;
        }

        // Add offline message if showing fallback results
        let headerMessage = '';
        if (options.showOfflineMessage) {
            headerMessage = `
                <div class="offline-notice" style="margin-bottom: 20px; padding: 15px; background: #d1ecf1; border: 1px solid #bee5eb; border-radius: 8px; color: #0c5460;">
                    <strong>📚 Offline Results:</strong> API unavailable. Showing results from offline dictionary.
                </div>
            `;
        }

        const resultsHTML = results.map(entry => this.createResultCard(entry, query)).join('');
        this.resultsContainer.innerHTML = headerMessage + resultsHTML;
...
};// Enhanced offline dictionary loading mechanism
async function mergeExternalJSON(url) {
  try {
    ...
    // Ensure global dictionary data is available
    window.dictionaryData = window.dictionaryData || dictionaryData;
    window.offlineDictionary = extra; // Store separate reference for offline use

    let added = 0, skipped = 0;
    for (const [originalKey, entry] of Object.entries(extra)) {
      // Don't lowercase Arabic keys - preserve original
      const key = originalKey.trim();
      if (!key || !entry) continue;

      // Use original key for both English and Arabic
      if (!window.dictionaryData[key]) {
        window.dictionaryData[key] = entry;
        added++;
      } else {
        skipped++;
      }
    ...
    console.log(`✓ Loaded offline dictionary: ${added} entries from ${url} (${skipped} already present)`);
    
    // Add offline indicator to UI
    if (added > 0) {
      addOfflineIndicator(true);
    }
  } catch (err) {
    console.error('Failed to load offline dictionary:', err);
    addOfflineIndicator(false);
  }
}

// Add UI indicator for offline dictionary status
function addOfflineIndicator(isLoaded) {
  ...
  indicator.style.cssText = `
    ...
    top: 20px;
    right: 20px;
    background: ${isLoaded ? '#28a745' : '#dc3545'};
    color: white;
    padding: 8px 12px;
    border-radius: 20px;
    font-size: 12px;
  ...
  `;
  indicator.innerHTML = `
    <span style="margin-right: 5px;">${isLoaded ? '📚' : '⚠️'}</span>
    ${isLoaded ? 'Offline Ready' : 'Offline Limited'}
  `;
  ...
  // Auto-hide after 3 seconds if loaded successfully
  if (isLoaded) {
    setTimeout(() => {
